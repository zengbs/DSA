...
\section*{Problem 3}
\begin{enumerate}
\item Pseudo code:\par
       \begin{minipage}{.8\linewidth}
       \begin{algorithm}[H]
          \SetKwFor{For}{for}{do}{endfor}
          \SetKwFunction{KwAppendEnd}{AppendEnd}
          \SetKwFunction{KwInitialList}{initalList}
          \SetKwIF{If}{ElseIf}{Else}{if}{then}{else if}{else}{endif}
          \SetKwArray{KwA}{A}
          \SetKwArray{KwL}{List}
          \KwIn{\KwA is the array containing $n$ integers $\in$ $\{0,1,\dots,n-1\}$}
          \KwIn{$i_{0}$ is the index where the frog is initially located.}

         \KwL = \KwInitialList{\KwA{$i_{0}$}}\; \textcolor{blue}{\tcc*[f]{initialize the list with the single node \KwA{$i_{0}$}}}

          \For{$(i=0; i < \KwA.Length; i++)$}{
             \If {i == \KwL.End}{\textcolor{blue}{\tcc*[f]{\KwL.End is the the last element of \KwL}}
                \Return{[Finite jumps!]}
             }
             \KwL = \KwAppendEnd{\KwL, \KwA{\KwL.End}}\;
             \textcolor{blue}{\tcc*[f]{append the node \KwA{\KwL.End} to the end of \KwL}}
          }
       ...
       \begin{itemize}
       \item Time complexity: The time complexity of loop itself is $\Theta(n)$ but \texttt{List.End},
       which is $\Theta(n)$, is inside the loop. Thus, the effective time complextity is \textcolor{blue}{$\Theta(n^2)$}.\\
       \item Space complexity: \textcolor{blue}{$\Theta(n)$} (Why?).
       \end{itemize}
...
\item Pseudo code:\par
       ...
       \begin{algorithm}[H]
          ...
          \SetKwFunction{KwAppendEnd}{AppendEnd}
          \SetKwFunction{KwInitialList}{initalList}
          \SetKwFunction{KwList}{List}
          \SetKwIF{If}{ElseIf}{Else}{if}{then}{else if}{else}{endif}
          \SetKwArray{KwA}{A}
          \SetKwArray{KwL}{List}
          \KwIn{\KwA is the array containing $n$ integers $\in$ $\{0,1,\dots,n-1\}$.}
          \KwIn{$i_{0}$ is the index where the frog is initially located.}
          \KwOut{$j$ is the length of the jumping loop.}

         \KwL = \KwInitialList{\KwA{$i_{0}$}}\;
         \textcolor{blue}{\tcc*[f]{initialize the list with the single node \KwA{$i_{0}$}}}

          \For{$(i=0$; $i<$ \KwA.Length; $i++)$}{
             \KwL = \KwAppendEnd{\KwL, \KwA{\KwL.End}}\;
             \textcolor{blue}{\tcc*[f]{append the node \KwA{\KwL.End} to the end of \KwL}}
             \For{$(j= \KwL.Length-2;j>=0; j--)$}{
                \If {\KwList{j} == \KwL.End}{\textcolor{blue}{\tcc*[f]{\KwList{j} is the $j^{\text{th}}$ element of \KwL}}\\
                       \Return{j}
                }
             }
          }
       ...
       \item 
       Given that the array $A$ is a strictly increasing array, $\text{max}(M_{0,i},M_{i,j},M_{j,n})$
       and $\text{min}(M_{0,i},M_{i,j},M_{j,n})$ must be equal to $M_{j,n}$ and $M_{0,i}$, respectively.
       Moreover, the two elements $A[i-1]$ and $A[j]$ must be as close as possible
       to make $f(i,j)=M_{j,n}-M_{0,i}$ reach the minimum. We thus simplify the original question as follows:
       \textcolor{blue}{find a index $k$, such that $M_{k+1,n}-M_{0,k-1}$ is minimized, where $i=k$ and $j=k+1$.}


       \begin{minipage}{.8\linewidth}
       \begin{algorithm}[H]
          \SetKwFor{For}{for}{do}{endfor}
          \SetKwFunction{KwGetMedian}{GetMedian}
          \SetKwFunction{KwGetMinIdx}{GetMinIdx}
          \SetKwIF{If}{ElseIf}{Else}{if}{then}{else if}{else}{endif}
          \SetKwArray{KwA}{A}
          \SetKwArray{KwF}{F}
          \KwIn{a strictly increasing array, \KwA.}
          \KwOut{$i, j$.}

          \For{$(m=1$; $m<$ \KwA.Length$-1$; $m++)$}{
               \KwF{m-1} = \KwGetMedian{m+1,n}-\KwGetMedian{0,m-1}\;
          }
...
Swap = true; /*Do not enter the outer loop if Swap=false,
which can help the time complexity in the worst case down to O(n)*/
Ptr = Head;

while( Ptr->Next != Head && Swap )
{
   Ptr_in = Head;
   Swap = false;
